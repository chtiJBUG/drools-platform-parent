DroolsPlatformControllers.controller('assetPackagingController', function ($rootScope, $scope, $http, $log) {
    ...
    $scope.deletePackageVersion = function () {
        var versionToDelete = $scope.newVersion;
        if($scope.isSnapshot) {
            versionToDelete+="-SNAPSHOT";
        }

        $http.delete('./server/rules_package/' + $scope.package + '/' + versionToDelete)
            .success(function (data) {
                // TODO show message to indicate that version has been deleted successfully
                //___ reload the list
                $scope.searchPackageByName();
            })
            .error(function () {
