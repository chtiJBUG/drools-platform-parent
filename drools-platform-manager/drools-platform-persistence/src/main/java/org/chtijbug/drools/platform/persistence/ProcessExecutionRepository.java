...
public interface ProcessExecutionRepository extends JpaRepository<ProcessExecution, Long> {
    ...
    List<ProcessExecution> findAllStartedProcessByRuleBaseIDAndSessionIDAndProcessInstanceId(@Param("ruleBaseID") Long ruleBaseID, @Param("sessionID") Long Long, @Param("processInstanceId") String processInstanceId);

    @Query("select r from PlatformRuntimeInstance p,ProcessExecution r,SessionExecution s where s.platformRuntimeInstance = p  and p.endDate is null  and r.sessionExecution = s and r.endDate is null and p.ruleBaseID = :ruleBaseID and s.sessionId = :sessionID and r.ProcessInstanceId = :processInstanceId")
    ProcessExecution findStartedProcessByRuleBaseIDBySessionIDAndProcessInstanceId(@Param("ruleBaseID") Long ruleBaseID, @Param("sessionID") Long sessionID, @Param("processInstanceId") String processInstanceId);
}
