...
public interface ProcessRuntimeRepository extends JpaRepository<ProcessRuntime, Long> {
    ...
    @Query("select r from PlatformRuntime p,ProcessRuntime r,SessionRuntime s where s.platformRuntime = p and r.sessionRuntime = s and r.endDate is null and p.ruleBaseID = :ruleBaseID and s.sessionId = :sessionID and r.ProcessInstanceId = :processInstanceId")
