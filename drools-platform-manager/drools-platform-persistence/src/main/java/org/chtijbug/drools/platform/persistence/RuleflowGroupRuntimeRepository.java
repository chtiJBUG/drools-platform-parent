...
public interface RuleflowGroupRuntimeRepository extends JpaRepository<RuleflowGroupRuntime, Long> {
    ...
    @Query("select r from PlatformRuntime pp,SessionRuntime s,ProcessRuntime p,RuleflowGroupRuntime r where s.platformRuntime=pp and pp.endDate is null and p.sessionRuntime=s and  r.processRuntime=p and pp.ruleBaseID= :ruleBaseID and s.sessionId =:sessionID  and p.ProcessInstanceId = :processInstanceID and r.endDate is null and r.ruleflowGroup = :ruleFlowGroupName")
    ...
    @Query("select r from PlatformRuntime pp,SessionRuntime s,ProcessRuntime p,RuleflowGroupRuntime r " +
            "where s.platformRuntime=pp  and pp.endDate is null and p.sessionRuntime=s and  r.processRuntime=p " +
    ...
    @Query("select r from PlatformRuntime pp,SessionRuntime s,ProcessRuntime p,RuleflowGroupRuntime r where s.platformRuntime=pp and pp.endDate is null and p.sessionRuntime=s and  r.processRuntime=p and pp.ruleBaseID= :ruleBaseID and s.sessionId =:sessionID  and r.endDate is null and r.ruleflowGroup = :ruleFlowGroupName")
