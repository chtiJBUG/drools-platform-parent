...
public interface RuleflowGroupRuntimeRepository extends JpaRepository<RuleflowGroupRuntime, Long> {
    ...
    @Query("select r from PlatformRuntimeInstance pp,SessionExecution s,ProcessExecution p,RuleflowGroupRuntime r where s.platformRuntimeInstance=pp and pp.endDate is null and p.sessionExecution=s and  r.processExecution=p and pp.ruleBaseID= :ruleBaseID and s.sessionId =:sessionID  and p.ProcessInstanceId = :processInstanceID and r.endDate is null and r.ruleflowGroup = :ruleFlowGroupName")
    ...
    @Query("select r from PlatformRuntimeInstance pp,SessionExecution s,ProcessExecution p,RuleflowGroupRuntime r " +
    ...
    @Query("select r from PlatformRuntimeInstance pp,SessionExecution s,ProcessExecution p,RuleflowGroupRuntime r where s.platformRuntimeInstance=pp and pp.endDate is null and p.sessionExecution=s and  r.processExecution=p and pp.ruleBaseID= :ruleBaseID and s.sessionId =:sessionID  and r.endDate is null and r.ruleflowGroup = :ruleFlowGroupName")
